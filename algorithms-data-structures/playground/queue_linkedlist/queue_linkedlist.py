...
class QueueLinkedList:
    ...
    def enqueue(self, item: any) -> void:
        node = DoublyNode()
    ...
    def dequeue(self) -> any:
        if self.__num_nodes == 0:
            raise RuntimeError("Attempt to dequeue empty queue")
        first_node = self.__head.get_next()
        self.__head.set_next(first_node.get_next())
        value = first_node.get_value()
